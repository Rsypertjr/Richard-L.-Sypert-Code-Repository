...
$conn=connectToMySQL("localhost","root","a",$mess);  //connect to MySQL
...
$sql = "CREATE TABLE protein (id int not null primary key auto_increment, locus varchar(75),speciesNumber varchar(75),speciesName varchar(75), accessionNumber varchar(75), proteinSequence varchar(175), proteinLength int)";
...
else
{
                ...
                print "Protein Table is Being built<br/>";
...
// Create mini-Motif database
   ...
   $sql = "CREATE TABLE miniMotif (id int not null primary key auto_increment,motifPattern varchar(75),actualMotif varchar(75),accessionNumber varchar(75),speciesName varchar(75), proteinLength int, motifLength int, startPosition int, endPosition int )";
    ...
    if(!$esql)
      ...
      {
       // Building MiniMotifs
             print "Minimotif Table is being built</br>";
...
if(isset($_GET["file"]) && !isset($_GET["motif"]))
  {
	...
	print "Database being checked....</br>";
	buildProteinDb($fileString);  // call function to Build Protein Database
	fclose($file); 
	print "Protein and Minimotif databases are built</br>";
  }
if(isset($_GET["motif"]))
  {
  ...
  $qstr = array();
  $qstr[] = 'SELECT accessionNumber, motifPattern, actualMotif FROM miniMotif WHERE motifPattern = "'.$moPatt.'" ORDER BY accessionNumber';
  $qstr[] = 'SELECT actualMotif, motifPattern, accessionNumber, motifLength FROM miniMotif WHERE motifPattern = "'.$moPatt.'"';
  $qstr[] = 'SELECT DISTINCT startPosition, motifPattern, accessionNumber FROM miniMotif WHERE motifPattern = "'.$moPatt.'"';
  $qstr[] = 'SELECT  DISTINCT accessionNumber, motifPattern FROM miniMotif WHERE motifPattern = "'.$moPatt.'" ORDER BY accessionNumber';
  $qstr[] = 'SELECT DISTINCT speciesName, motifPattern FROM miniMotif WHERE motifPattern = "'.$moPatt.'"';
  ...
  $qu = array(array('accessionNumber','motifPattern','actualMotif',''),
  ...
  $qu1 = array(array('Accession Number','Motif Pattern','Actual Motif',''),
               array('Actual Motif<br/>(NOTE: Does not always show whole sequence, i.e. check Motif length)','Motif Pattern','Accession Number','Motif Length'),
